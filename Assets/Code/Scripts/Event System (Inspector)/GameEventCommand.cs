...
public class GameEventCommand : ScriptableObject
{
    ...
    {
        GlobalHelper.Print<GameEventCommand>("Triggerig... " + listeners.Count);
        for (int i = listeners.Count - 1; i >= 0; i--)
            listeners[i].OnEventTriggered(command);
    }

    public void AddListener(GameEventCommandListener listener)
    {
        listeners.Add(listener);
    }
    
    public void RemoveListener(GameEventCommandListener listener)
    {
